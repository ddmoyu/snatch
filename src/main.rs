...
fn ensure_configs(app_dir: &Path) { std::fs::create_dir_all(app_dir).ok(); for (name, default) in [("settings.toml", DEFAULT_SETTINGS), ("scraper.toml", DEFAULT_SCRAPER)] { let p = app_dir.join(name); let ex = app_dir.join(format!("{}.example", name)); if !p.exists() { if ex.exists() { let _ = std::fs::copy(&ex, &p); } else { std::fs::write(&p, default).expect("create config"); } } } }
